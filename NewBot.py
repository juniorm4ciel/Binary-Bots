...
class RoboThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            if quadrante != ultimo_quadrante and agora.minute % 5 == 0:
                ...
                # Dorme até o próximo minuto para garantir que só vai operar no próximo quadrante
                agora = datetime.datetime.now()
                time.sleep(60 - agora.second)
            else:
                # Dorme até o próximo minuto para não ocupar CPU à toa
                agora = datetime.datetime.now()
                time.sleep(60 - agora.second)
