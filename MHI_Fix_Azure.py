...
class MHIRobot:
    ...
    def obter_adx(self, ativo):
        # Pega candles suficientes para o cálculo do ADX
        candles = self.get_candles(ativo, n=ADX_PERIOD+2, size=60)
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            if quadrante != ultimo_quadrante and agora.minute % 5 == 0 and agora.second < 2:
                ...
                # Checagem de ADX antes de pegar o sinal
                adx_usar = self.config.get('adx', True)
                adx_valor = None
                adx_bloqueado = False
                if adx_usar:
                    adx_valor = self.obter_adx(ativo)
                    if adx_valor is not None:
                        self.log(f"[{ativo}] ADX no momento: {adx_valor:.2f}", "#00BFFF")
                        if adx_valor > ADX_LIMIAR:
                            self.log(f"[{ativo}] Operação BLOQUEADA pelo ADX ({adx_valor:.2f} > {ADX_LIMIAR})", "#FF4040")
                            adx_bloqueado = True
                    else:
                        self.log(f"[{ativo}] Não foi possível calcular o ADX (candles insuficientes).", "#FF4040")
                        adx_bloqueado = True
                if adx_bloqueado:
                    continue
    ...
    def catalogar_ativo(self):
        ...
        if selecionados:
            for ativo in selecionados:
                try:
                    # Adiciona log do ADX na catalogação
                    candles = self.api.get_candles(ativo, 60, ADX_PERIOD+6)
                    adx, _, _ = calculate_adx(candles)
                    cat, rep = self.api.catalogar_mhi(ativo, minutos=50)
                    if adx is not None:
                        self.log_event(f"[{ativo}] ADX na análise de assertividade: {adx:.2f}", "#00BFFF")
                    else:
                        self.log_event(f"[{ativo}] Não foi possível calcular o ADX (candles insuficientes).", "#FF4040")
                    self.log_event(cat, "#FFD700")
                    self.log_event(rep, "#FFA500")
                except Exception as e:
                    self.log_event(f"Erro ao catalogar {ativo}: {e}", "#FF4040")
        else:
            melhor = {"ativo": None, "cat": "", "rep": "", "assertividade": -1, "adx": None}
            for ativo in self.ativos:
                try:
                    candles = self.api.get_candles(ativo, 60, ADX_PERIOD+6)
                    adx, _, _ = calculate_adx(candles)
                    ...
                    if perc > melhor["assertividade"]:
                        melhor = {"ativo": ativo, "cat": cat, "rep": rep, "assertividade": perc, "adx": adx}
                except Exception:
                    continue
            if melhor["ativo"]:
                if melhor["adx"] is not None:
                    self.log_event(f"[{melhor['ativo']}] ADX na análise de assertividade: {melhor['adx']:.2f}", "#00BFFF")
                else:
                    self.log_event(f"[{melhor['ativo']}] Não foi possível calcular o ADX (candles insuficientes).", "#FF4040")
    ...
    def start_robot(self):
        ...
        try:
            config = {
                ...
                "adx": self.var_adx.get(),  # ativar filtro ADX
